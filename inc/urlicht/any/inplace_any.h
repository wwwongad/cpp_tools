...
#ifndef INPLACE_ANY_H
#define INPLACE_ANY_H
...
namespace urlicht {
    ...
    */
    ...
    private:
        ...
        constexpr void copy_from(const inplace_any& other) {
            try {
                ...
                vtable_->clone(other.data_, data_);
            } catch (...) {
                vtable_ = nullptr;
                throw;
            }
        ...
        constexpr void move_from(inplace_any&& other) {
            vtable_ = other.vtable_;
            vtable_->move(other.data_, data_);
    ...
    public:
        ...
        constexpr void swap(inplace_any& other) noexcept {
            if (this == &other) {
                return;
            }
        ...
        friend const T* any_cast(const inplace_any<S, A>* operand) noexcept;

        template <typename T, std::size_t S, std::size_t A>
        friend T* any_cast(inplace_any<S, A>* operand) noexcept;

        template <typename T, std::size_t S, std::size_t A>
        friend T* unchecked_any_cast(inplace_any<S, A>* operand) noexcept;
    ...
    template <typename T, std::size_t S, std::size_t A>
    T* any_cast(inplace_any<S, A>* operand) noexcept {
        if (operand && operand->has_value() && operand->type_info() == typeid(T)) {
            return reinterpret_cast<T*>(operand->data_);
        }
        return nullptr;
    }

    template <typename T, std::size_t S, std::size_t A>
    const T* any_cast(const inplace_any<S, A>* operand) noexcept {
        return any_cast<T>(const_cast<inplace_any<S, A>*>(operand));
    }

    template <typename T, std::size_t S, std::size_t A>
    T& any_cast(inplace_any<S, A>& operand) {
    ...
    template <typename T, std::size_t S, std::size_t A>
    const T& any_cast(const inplace_any<S, A>& operand) {
        return any_cast<T>(const_cast<inplace_any<S, A>&>(operand));
    }
    ...
    template <typename T, std::size_t S, std::size_t A>
    T* unchecked_any_cast(inplace_any<S, A>* operand) noexcept {
        return reinterpret_cast<T*>(operand->data_);
    }
    ...
    template <typename T, std::size_t S, std::size_t A>
    const T& unchecked_any_cast(const inplace_any<S, A>& operand) noexcept {
    ...
    // make_inplace_any
    template <typename T, typename... Args, std::size_t S = sizeof(T) * 2, std::size_t A = alignof(T)>
    requires (sizeof(T) > 0)
    auto make_inplace_any(Args&&... args) {
