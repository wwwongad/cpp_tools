...
namespace urlicht::concepts {
	...
	template<typename Compare, typename Value>
	concept compatible_functor =
		requires(Compare comp, Value val) {
	...
	concept nothrow_equality_comparable =
		concepts::equality_comparable<T, U> && noexcept(std::declval<T>() == std::declval<U>());

	template <typename T, typename U = T>
	concept nothrow_inequality_comparable =
		concepts::inequality_comparable<T, U> && noexcept(std::declval<T>() != std::declval<U>());

	template <typename T, typename U = T>
	concept nothrow_less_comparable =
		concepts::less_comparable<T, U> && noexcept(std::declval<T>() < std::declval<U>());

	template <typename T, typename U = T>
	concept nothrow_greater_comparable =
		concepts::greater_comparable<T, U> && noexcept(std::declval<T>() > std::declval<U>());

	template <typename T, typename U = T>
	concept nothrow_three_way_comparable =
		std::three_way_comparable_with<T, U> && noexcept(std::declval<T>() <=> std::declval<U>());
	...
	&& requires(Cont& c,
		...
		{ c.emplace_front(val) } -> std::same_as<typename Cont::reference>;
